"""
KV-CACHED VARIANT of persona_analysis/analyze_personas.py.

Identical analysis, but reads the reward-model scores produced by the
`--kv-cache` scoring path:
  baseline: data/reward_model_scores_kv_cached/<model>.csv
  persona:  data/persona_reward_model_scores_kv_cached/<model>.csv
and writes its outputs to kv_analysis/output/ (instead of
persona_analysis/output/), so the kv-cached figures notebook
(kv_analysis/gen_figures_kv.ipynb) can consume them without touching the
original main-pipeline outputs. Base-LM logit inputs are unchanged.

Run from anywhere:  python kv_analysis/analyze_personas_kv.py
...
baseline ranking (data/reward_model_scores_kv_cached/<model>.csv) using Kendall's tau,
...
for model_info in model_configs:
    ...
    # KV-cached variant: both the neutral baseline and the persona-conditioned
    # scores come from the --kv-cache scoring path (the bug-fixed, prefix-cached
    # pipeline) so the persona-vs-baseline shift is measured entirely within the
    # kv-cached "world" — matching how tau_kv/rbo_kv are computed in
    # colab_run_pipeline.ipynb 7c. Base-LM logits (below) are unchanged: the
    # base LM isn't re-scored under kv-caching.
    baseline_path = DATA_DIR / 'reward_model_scores_kv_cached' / f"{safe_name}.csv"
    persona_path = DATA_DIR / 'persona_reward_model_scores_kv_cached' / f"{safe_name}.csv"
